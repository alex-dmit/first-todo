...
if (tasksElem) {
    tasksElem.addEventListener('click', function(event) {
        ...
        const checkInputElem = target.closest<HTMLInputElement>('.check-input[type=checkbox]')
        if (checkInputElem) {
...
function toggleTextDecor(checkbox: HTMLInputElement) {
    ...
    const taskNameElem = checkbox.parentElement!.nextElementSibling as HTMLDivElement
    if (checkbox.checked) {
